const screenBeforePressingButton = document.querySelector('.screenBeforePressingButton');
const screenAfterPressingButton = document.querySelector('.screenAfterPressingButton');
...
btnOpenAnotherCookie.addEventListener('click', OpenAnotherCookie);
...
function openCookie() {
  selectedSentence = phraseSelector();
  screenAfterPressingButton.querySelector('p').innerText = selectedSentence;
  toggleScreen();
}
...
function phraseSelector() {
  let randomPhrase = Math.round(Math.random() * phraseList.length);
  return phraseList[randomPhrase];
}

const toggleScreen = () => {
  screenBeforePressingButton.classList.toggle('hide');
  screenAfterPressingButton.classList.toggle('hide');
}

function handleKeypress(e) {
  if(e.key == 'Enter' && screenBeforePressingButton.classList.contains('hide')) {
    OpenAnotherCookie()
  } else if (e.key == 'Enter' && screenAfterPressingButton.classList.contains('hide')){
    openCookie()
  }
}